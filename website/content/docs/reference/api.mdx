...
| `result.probabilities` | `dict[str, float]` | Outcome probabilities (populated on every backend; exact from the state when available, otherwise normalized counts) |
...
MPS `shots=0` densifies the tensor network into a full statevector. For more than 26 qubits this raises a `MemoryError` with guidance — pass `shots>0` to sample directly from the tensor network instead. MPS sampling and statevector output use the same little-endian qubit order as the statevector backend (`"001"` means qubit 0 is `|1⟩`).
...
State operations that the stabilizer backend does not support (`.numpy()`, `.partial_trace()`, `.fidelity()`, `.entropy()`, `.purity()`, `.probabilities()`, `.grad()`) raise `superfermion.MethodError`, which is a subclass of `RuntimeError` — catching either works.
...
`counts`/`probabilities` keys follow the shared little-endian key convention (qubit `q` = bit `q` of `int(key, 2)`); `result.metadata["density_matrix"]` is big-endian (qubit 0 = most significant). See the execution guide for the full layout notes.
