    ...
    fn run(&self, dag: &mut QuantumDAG) -> Result<(), CompilerError> {
        ...
        for (node_id, op_type) in to_replace {
            ...
            match op_type {
                ...
                OpType::CCX => {
                    return Err(CompilerError::UnsupportedGate("CCX/Toffoli".to_string()));
                }
                _ => {}
            }
        }
...
impl BasicDecomposePass {
    fn replace_node_with_gates(
        ...
        gates: &[(OpType, Vec<sf_ir::QubitId>)],
    ) {
        ...
        for (new_op, qs) in gates {
            let new_node = dag
                .graph_mut()
                .add_node(sf_ir::QuantumOp::new(new_op.clone(), qs));
            for &q in qs {
                let p = current_preds[&q];
                dag.graph_mut()
                    .add_edge(p, new_node, sf_ir::WireType::Qubit(q));
        ...
        for &q in &qubits {
            ...
            let target = succs[&q];
            dag.graph_mut()
                .add_edge(last_node, target, sf_ir::WireType::Qubit(q));
        }
