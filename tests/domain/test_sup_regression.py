...
class TestProbabilitiesPopulated:
    ...
    def test_density_matrix_keys_little_endian(self):
        """(f) DM counts/probabilities keys use the shared LE key convention
        (qubit q = bit q), like statevector/MPS/default.  Regression: the DM
        reads were bit-reversed vs every other method (x(2)@n=3 -> '001'
        instead of '100')."""
        rp = sf.run(Circuit(3).x(2), method="density_matrix", shots=0, seed=7)
        assert set(rp.probabilities) == {"100"}
        rc = sf.run(Circuit(3).x(2), method="density_matrix", shots=1000, seed=7)
        assert set(rc.counts) == {"100"}
        rh = sf.run(Circuit(3).h(0), method="density_matrix", shots=0, seed=7)
        assert set(rh.probabilities) == {"000", "001"}
